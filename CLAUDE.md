...
- Web: `wasmJs` (Kotlin/Wasm), deployed as a static site to GitHub Pages by `.github/workflows/deploy-pages.yml` on every push to the default branch. PDF import and text recognition go through CDN-loaded JS libraries there rather than a platform engine (see "Web target" below).
