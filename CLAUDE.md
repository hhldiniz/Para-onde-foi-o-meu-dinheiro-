...
A fixed-income position also records **what it was contracted to pay** — `Investment.yieldMode` (a `YieldMode` from `domain/model/InvestmentYield.kt`: prefixed, a percentage of the CDI, CDI/IPCA/IGP-M/Selic plus a spread, or none) together with `yieldRate`, the number that goes with it. It is descriptive and nothing recalculates from it: "110% do CDI" is what the paper promises, `currentValue` is still whatever the user last typed, since the app quotes no index. The two are stored together because the rate means something different in each mode, and both are required or neither is — "IPCA + ?" says nothing, so `save()` rejects a mode whose rate will not parse. The form offers the pair only for types where `InvestmentType.supportsYield` is true (fixed income), and switching to a type that has no contracted rate clears what was typed rather than saving a hidden field. Like `InvestmentType`, `YieldMode` persists as a stable `key` and `fromKey` falls back to `NONE`; display names and the "110% do CDI" templates live in `presentation/components/InvestmentYieldLocalization.kt`.
...
- `data/local/AppDatabase.kt` (the `@Database` abstract class, `AppDatabaseConstructor` expect/actual, `buildAppDatabase()`, `DatabaseBuilderFactory` expect) is otherwise unchanged from before the wasmJs work, just relocated. It's version 5 (bumped by the `investments` table, then by that table's contracted-yield columns) with `fallbackToDestructiveMigration(dropAllTables = true)` — schema changes do not need a migration path, the DB is just recreated (acceptable since this is unsynced local data, though it does mean an Android/iOS user's imported entries are dropped on such an upgrade; wasmJs, having no Room, keeps its `localStorage` data). One migration is declared anyway: `MIGRATION_4_5` adds `yield_mode`/`yield_rate` with two `ALTER TABLE`s, because dropping a user's whole portfolio and import history over two appended columns would be gratuitous. Add one the same way when a change is that cheap; the destructive fallback stays as the net for the ones that aren't. Room's KSP processor generates `AppDatabaseConstructor`'s `actual` per target (hence `kspAndroid` / `kspIos*` in `app/build.gradle.kts`; KSP needs no separate wiring for `roomMain` itself, since it processes each compilation's full merged source set graph, and `androidMain`/`iosMain` already `dependsOn(roomMain)`). `DatabaseBuilderFactory` supplies the platform's `RoomDatabase.Builder` (a `Context` on Android, the Documents directory on iOS) and `buildAppDatabase()` finishes the wiring. On first creation the callback seeds a fixed set of Portuguese category names (Alimentacao, Transporte, Lazer, etc.) — see "Web target" below for how wasmJs, which has no such callback, ends up with the same seed data anyway.
...
- `InvestmentsViewModelTest` drives the investments tab through a real `InvestmentRepository` over an in-memory `InvestmentDao` (a mock would not re-emit its Flow, which is where the tab's totals come from); `data/local/entity/InvestmentEntityTest` covers the derived profit numbers and both persisted representations, and pins `InvestmentType`'s and `YieldMode`'s stored keys — changing one silently reinterprets rows already on disk. It also pins what a row written *before* the contracted-yield fields existed reads back as, in both representations (a missing Room column defaulting to `none`/null, a JSON object without the properties), which is the half of an added field that is easy to get wrong.
